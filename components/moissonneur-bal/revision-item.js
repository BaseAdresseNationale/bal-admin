...
const RevisionItem = ({
  ...
  const displayForcePublishButton =
    !publication ||
    publication?.status === "provided-by-other-source" ||
    publication?.status === "provided-by-other-client";
  return (
    <tr>
      {!ignoreFields.includes("id") && (
      ...
      {!ignoreFields.includes("nbRows") && (
        <td className="fr-col fr-my-1v">
          <a>{nbRows}</a>
        </td>
      )}
      {!ignoreFields.includes("nbRowsWithErrors") && (
        <td className="fr-col fr-my-1v">
          <a>{nbRowsWithErrors}</a>
        </td>
      )}
      {!ignoreFields.includes("updateStatus") && (
        <td className="fr-col fr-my-1v">
          <UpdateStatusBadge
      ...
      {!ignoreFields.includes("publication") && (
        <td className="fr-col fr-my-1v">
          <RevisionPublication {...publication} />
        </td>
      )}
      {!ignoreFields.includes("file") && (
        <td className="fr-col fr-my-1v">
          {fileId && (
            <Button
